...
static CODEUR Codeurs [NO_OF_OUTPUT_PORTS];
...
{
  if(!Power) SetDuty[Port](0);//#Champi
...
void StopAndBrakeMotor(UBYTE MotorNo)
{
  ...
  printk("Motor No %d, Power=%d\n",MotorNo,Motor[MotorNo].Power);
  SetBrake(MotorNo);
}
...
static int Device1Init(void)
{
  ...
  else
  {
    ...
    for(Tmp = 0; Tmp < NO_OF_OUTPUT_PORTS; Tmp++)
    {
      ...
      Codeurs[Tmp].valPrec=0;//#Champi : petite imprécision au départ, mais il faut bien une initialisation, peut-être penser à qqch de plus propre
...
static irqreturn_t IntA (int irq, void * dev)//channel A
{
  ...
  Codeurs[0].voieA=(READIntA)?1:0;
    
    if (!Codeurs[0].voieA)
    {
      if(!Codeurs[0].voieB)//3
      {
	if(Codeurs[0].valPrec & 1)
	{
...
{
  // Sample all necessary items as fast as possible
  Codeurs[1].voieA=(READIntB)?1:0;
    
    if (!Codeurs[1].voieA)
    {
      if(!Codeurs[1].voieB)//3
      {
	if(Codeurs[1].valPrec & 1)
	{
...
{
  // Sample all necessary items as fast as possible
  Codeurs[2].voieA=(READIntC)?1:0;
    
    if (!Codeurs[2].voieA)
    {
      if(!Codeurs[2].voieB)//3
      {
	if(Codeurs[2].valPrec & 1)
	{
...
{
  // Sample all necessary items as fast as possible
  Codeurs[3].voieA=(READIntD)?1:0;
    
    if (!Codeurs[3].voieA)
    {
      if(!Codeurs[3].voieB)//3
      {
	if(Codeurs[3].valPrec & 1)
	{
